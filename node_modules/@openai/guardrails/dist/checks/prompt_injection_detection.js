...
exports.promptInjectionDetectionCheck = exports.PromptInjectionDetectionOutput = exports.PromptInjectionDetectionBaseOutput = exports.PromptInjectionDetectionConfig = void 0;
...
exports.PromptInjectionDetectionConfig = zod_1.z.object({
     ...
     */
    include_reasoning: zod_1.z
        .boolean()
        .default(false)
        .describe('Whether to include detailed reasoning fields in the output. Defaults to false to minimize token costs.'),
    /**
    ...
    max_turns: zod_1.z
        ...
        .describe('Maximum number of conversation turns to include for multi-turn analysis. Defaults to 10. Set to 1 for single-turn mode.'),
});
/**
 * Base output schema for prompt injection detection (without reasoning fields).
 */
exports.PromptInjectionDetectionBaseOutput = llm_base_1.LLMOutput;
/**
 ...
 */
function buildPromptInjectionDetectionPrompt(includeReasoning) {
    if (includeReasoning) {
...
const STRICT_JSON_INSTRUCTION = 'Respond with ONLY a single JSON object containing the fields above. Do not add prose, markdown, or explanations outside the JSON.';
/**
...
const promptInjectionDetectionCheck = async (ctx, data, config) => {
    try {
        ...
        const { recentMessages, actionableMessages, userIntent } = prepareConversationSlice(conversationHistory, parsedDataMessages, maxTurns);
        ...
        const analysisPrompt = buildAnalysisPrompt(userGoalText, recentMessages, actionableMessages, includeReasoning);
        const { analysis, tokenUsage, executionFailed, errorMessage } = await callPromptInjectionDetectionLLM(ctx, analysisPrompt, config);
        ...
        const resultInfo = {
            ...
            token_usage: (0, types_1.tokenUsageToDict)(tokenUsage),
        };
...
function prepareConversationSlice(conversationHistory, parsedDataMessages, maxTurns) {
...
const SKIPPED_USAGE = Object.freeze({
    prompt_tokens: null,
    completion_tokens: null,
    total_tokens: null,
    unavailable_reason: 'No LLM call made (check was skipped)',
});
function createSkipResult(observation, threshold, recentMessagesJson, userGoal = 'N/A', action = [], recentMessages = [], tokenUsage = SKIPPED_USAGE) {
    return {
        tripwireTriggered: false,
        info: {
            guardrail_name: 'Prompt Injection Detection',
...
function buildAnalysisPrompt(userGoalText, recentMessages, actionableMessages, includeReasoning) {
...
async function callPromptInjectionDetectionLLM(ctx, prompt, config) {
    ...
    const selectedOutputModel = includeReasoning
        ? exports.PromptInjectionDetectionOutput
        : exports.PromptInjectionDetectionBaseOutput;
    // Build fallback output with reasoning fields if reasoning was requested
    const fallbackOutput = includeReasoning
        ? {
            flagged: false,
            confidence: 0.0,
    ...
    try {
        const [result, tokenUsage] = await (0, llm_base_1.runLLM)(prompt, '', ctx.guardrailLlm, config.model, selectedOutputModel);
...
registry_1.defaultSpecRegistry.register('Prompt Injection Detection', exports.promptInjectionDetectionCheck, "Guardrail that detects when tool calls or tool outputs contain malicious instructions not aligned with the user's intent. Parses conversation history and uses LLM-based analysis for prompt injection detection checking.", 'text/plain', exports.PromptInjectionDetectionConfig, undefined, { engine: 'LLM', usesConversationHistory: true });
