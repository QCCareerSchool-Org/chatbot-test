...
(0, vitest_1.describe)('GuardrailAgent', () => {
    ...
    (0, vitest_1.it)('propagates guardrail metadata to outputInfo on success', async () => {
        process.env.OPENAI_API_KEY = 'test';
        const config = {
            version: 1,
            input: {
        ...
        const agent = (await agents_1.GuardrailAgent.create(config, 'Metadata Agent', 'Test instructions'));
        const guardrailFunction = agent.inputGuardrails[0];
        const result = await guardrailFunction.execute('payload');
        (0, vitest_1.expect)(result.tripwireTriggered).toBe(false);
        (0, vitest_1.expect)(result.outputInfo.guardrail_name).toBe('Jailbreak');
        (0, vitest_1.expect)(result.outputInfo.token_usage).toEqual({
            prompt_tokens: 42,
            completion_tokens: 10,
            total_tokens: 52,
        });
