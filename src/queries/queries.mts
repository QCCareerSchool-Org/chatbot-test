...
const outputGuardrailsConfig: GuardrailBundle = {
  guardrails: [
    { name: 'Hallucination Detection', config: { model: 'gpt-4.1-mini', knowledge_source: 'vs_6a0f10bd9c60819181d27ab29297f270', confidence_threshold: 0.7 } },
    { name: 'URL Filter', config: { url_allow_list: [ 'example.com', '192.168.1.100', 'https://api.service.com/v1' ], allowed_schemes: [ 'https' ], block_userinfo: true, allow_subdomains: false } },
...
function triggeredGuardrail(results: GuardrailResult[], name: string): boolean {
  return results.some(r => r.tripwireTriggered && r.info.guardrail_name === name);
}
...
export async function generateResponse(userInput: string): Promise<string> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    const hallucinated = triggeredGuardrail(outputGuardrails, 'Hallucination Detection');
...
function hasTripwire(results: GuardrailResult[]): boolean {
  return results.some(r => r.tripwireTriggered);
}

// function getSafeText(results: GuardrailResult[], fallbackText: string): string {
//   for (const r of results) {
//     if ('checked_text' in r.info) {
//       return r.info.checked_text ?? fallbackText;
//     }
//   }
//   const pii = results.find(r => 'anonymized_text' in r.info);
//   if (pii && 'anonymized_text' in pii.info && pii.info.anonymized_text === 'string') {
//     return pii.info.anonymized_text;
//   }
//   return fallbackText;
// }
